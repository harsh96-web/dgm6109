...
/*
 * ===================================================================
 * FITNESS ENERGY DASHBOARD - BATTERY METAPHOR VISUALIZATION
 * ===================================================================
 * Harsh Kumar
...
let SVG_WIDTH = 1600;             
let SVG_HEIGHT = 1600;           // height for legend + 30 batteries

let BREATHING_ROOM = 64;        // left/right margin inside the SVG
...
function createBatteryDashboard(dataset) {
    ...
    // Pre-compute x,y positions with horizontal breathing room
    let positions = [];
    let totalRows = Math.ceil(dataset.length / BATTERIES_PER_ROW);
    let startY = 100;   // vertical offset for the first row
    ...
    for (let row = 0; row < totalRows; row++) {
        ...
        // Available space inside left/right margin
        let availableWidth = SVG_WIDTH - (BREATHING_ROOM * 2);

        // Gap so the whole row fits between the margins
    ...
    // Top label: sleep percentage (based on fill)
    groups.append("text")
    ...
    // Battery fill (height = sleep; color = restfulness)
    groups.append("rect")
    ...
    // Sleep hours text under battery
    groups.append("text")
        .attr("x", function (d) { return widthScale(d.protein) / 2; })
    ...
    // Star marker for days with maximum restfulness (restfulness = 5)
    let starGroup = groups
        .filter(function (d) { return d.restfulness === 5; })
        .append("g")
...
// ===================================================================
// LEGEND — layout with explicit units
// ===================================================================
function createLegend() {
    ...
    // Title: includes units for all mapped variables
    legend.append("text")
    ...
    fillG.append("rect")
        ...
        .attr("stroke", "black")
        .attr("stroke-width", 2);
    fillG.append("rect") // terminal
        .attr("x", 7).attr("y", -4)
        .attr("width", 10).attr("height", 6)
        .attr("fill", "black");
    fillG.append("rect") // low fill
        .attr("x", 2).attr("y", 23)
